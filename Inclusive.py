...
nx = 2;
xlimits = [(0.,60.)];
ylimits = [(-2.7*1e-7,3.4*1e-7),(-0.1,5)];
xtitle = ["$p_{T}$","$\\eta$"];
ytitle = ["$1/N_{evt} dN_{ch}/dp_T$","$1/N_{evt} dN_{ch}/d\\eta$"];
Nevt = 7640.0;
...
hist_pt = f.Get("lauras-code/hChargedPt")
hist_eta = f.Get("lauras-code/hDeltaEta")

hist_pt.Scale(1./Nevt,"width")
hist_eta.Scale(1./Nevt,"width")
